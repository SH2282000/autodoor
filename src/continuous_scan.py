    ...
    def device_callback(device, advertisement_data):
        ...
        if device.name == name:
            ...
            if (absence_duration > ABSENCE_THRESHOLD):
                print(f"Hello again! absence duration: {absence_duration:.2f}s, current time: {time.strftime("%H:%M:%S", time.localtime(current_time))} RSSI: {advertisement_data.rssi} dBm")
